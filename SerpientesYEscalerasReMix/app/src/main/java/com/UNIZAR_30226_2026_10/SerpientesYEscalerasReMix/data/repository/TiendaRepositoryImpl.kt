...
class TiendaRepositoryImpl(
    ...
    override suspend fun getProductos(email: String): List<Producto> {
        ...
        if (!response.isSuccessful) {
            throw IllegalStateException("Error getProductos: ${response.code()}")
        }
    ...
    override suspend fun comprarProducto(email: String, producto: Producto): Boolean {
        ...
        if (response.isSuccessful) {
            ...
            throw IllegalStateException("Error comprarProducto: ${response.code()}")
        }
        return response.isSuccessful
    }

    override suspend fun getSaldo(email: String): Int {
        val response = apiService.getSaldo(email)
        if (!response.isSuccessful) {
            throw IllegalStateException("Error getSaldo: ${response.code()}")
        }
