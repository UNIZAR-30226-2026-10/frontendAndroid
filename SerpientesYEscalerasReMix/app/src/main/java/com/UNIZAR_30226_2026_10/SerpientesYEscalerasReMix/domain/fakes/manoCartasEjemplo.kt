...
val fakeManoCartas = MutableList<Carta?>(4) { idx -> // Usa llaves, no paréntesis
    when(idx) {
        0 -> Carta("Wild Frank", "Poner una serpiente", tipo = Tipo_Carta.Entorno, calidad = Calidad.Epica)
        1 -> Carta("Pickpocket", "Roba una carta al azar de otro jugador", tipo = Tipo_Carta.Ofensiva, calidad = Calidad.Rara)
        2 -> Carta("Robo de identidad", "Cambia una de tus fihcas por otra al azar", tipo = Tipo_Carta.Defensiva, calidad = Calidad.Comun)
        else -> null
    }
}